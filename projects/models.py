...
class ProjectSnapshotStatus(models.TextChoices):
    DRAFT = "draft", "Draft"
    FINAL = "final", "Final"

class ProjectSnapshot(TimeStamped, Owned):
    """
    One snapshot row per Project, used for backup/exports.

    - Created when the Project is created.
    - Stores client, contract, invoice, event summary.
    - Deliverables are added when project is marked completed.
    - Invoice/payment totals auto-update when payments are added.
    """
    ...
    invoice_numbers = models.TextField(
        blank=True,
        help_text="Comma-separated invoice numbers for this project.",
    )
    ...
    is_fully_paid = models.BooleanField(
        default=False,
        help_text="True when payments_total >= invoice_total.",
    )
    ...
    main_event = models.ForeignKey(
        ...
        null=True,
        blank=True,
        help_text="Primary event for this project, if any.",
        related_name="main_event_snapshots",
    )
    event_dates = models.TextField(
        blank=True,
        help_text="Comma-separated event dates (YYYY-MM-DD).",
    )
    event_venues = models.TextField(
        blank=True,
        help_text="Comma-separated venue names.",
    )

    # --- Deliverables snapshot ---
    # Filled when project is set to completed.
    deliverables_data = models.JSONField(
        default=list,
        blank=True,
        help_text="List of deliverables with name/type/status/file_link.",
    )

    status = models.CharField(
        max_length=20,
        choices=ProjectSnapshotStatus.choices,
        default=ProjectSnapshotStatus.DRAFT,
    )
    ...
    def __str__(self):
        return f"Snapshot for {self.project} ({self.client_name or 'No client'})"
